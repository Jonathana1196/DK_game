...
void lifes() {
    for (int i; i < life; i++) {
        ...
        SDL_Surface *tempL = IMG_Load("../Data/Images/mario.png");
...
void death() {
    for (int i = 0; i < bN; i++) {
        if ((barrilesN[i].x - 10) < windowRectM.x && windowRectM.x < (barrilesN[i].x + 10) &&
            (barrilesN[i].y - 10) < windowRectM.y && windowRectM.y < (barrilesN[i].y + 10)) {
            Mix_Chunk *death = Mix_LoadWAV("../Data/Sounds/death.wav");
    ...
    for (int i = 0; i < bC; i++) {
        if ((barrilesC[i].x - 10) < windowRectM.x && windowRectM.x < (barrilesC[i].x + 10) &&
            (barrilesC[i].y - 10) < windowRectM.y && windowRectM.y < (barrilesC[i].y + 10)) {
            Mix_Chunk *death = Mix_LoadWAV("../Data/Sounds/death.wav");
            Mix_PlayChannel(-1, death, 0);
            life -= 1;
            bC = 0;
...
void eventos(SDL_Event event) {
    ...
    if (event.key.keysym.sym == SDLK_RIGHT && !stairs) {
        stop = true;
        tempM = IMG_Load("../Data/Images/marioML.png");
        spriteSheetM = SDL_CreateTextureFromSurface(renderer, tempM);
        textureRectM.w = 18;
        int totalFramesM = 2;
        ...
        onStairs();
        if (in) {
            tempM = IMG_Load("../Data/Images/marioU.png");
            spriteSheetM = SDL_CreateTextureFromSurface(renderer, tempM);
            textureRectM.w = 15;
            int totalFramesM = 2;
            int delayPerFrameM = 100;
...
void ventana() {
    ...
    Mix_Music *back = Mix_LoadMUS("../Data/Sounds/bacmusic.wav");
    ...
    background = SDL_LoadBMP("../Data/Images/background.bmp");
